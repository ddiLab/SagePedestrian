                ...
                <form action="restructure.php" method="GET" id="date">
                    ...
                    <?php
                            ...
                            {   
                                ...
                                echo '<input class="dateform" id="dateinput" type="text" name="date" style="margin: 0; font-size: 22px;" placeholder="Click to choose a date" />';
                                #echo "<script>var dates = " . json_encode($output1[0]) . ";</script>";
...
function set_query($date, $xwalk_opts, $timestop, $timestart) {
    ...
    $opt = (int)$xwalk_opts;
        switch($opt) {
            case 0://WORKING!
                $return_query = "SELECT PERMAID,XCOORD,YCOORD from Coordinate where DATE >= " . "'" . $time_start . "'" . " and DATE <= " . "'" . $time_stop . "'" . ';';
                break;
            case 1://WORKING!
                $return_query = "SELECT PERMAID, XCOORD, YCOORD from Coordinate where PERMAID in (select PERMAID from Person where USEROAD=0 and USECROSSWALK=0 intersect select PERMAID from Contains where DATE >= " . "'" . $time_start . "'" . " and DATE <= " . "'" . $time_stop . "');";
                break;
            case 2://WORKING!
                $return_query = "SELECT PERMAID, XCOORD, YCOORD from Coordinate where PERMAID in (select PERMAID from Person where USEROAD=1 and USECROSSWALK=0 intersect select PERMAID from Contains where DATE >= " . "'" . $time_start . "'" . " and DATE <= " . "'" . $time_stop . "');";
                break;
            case 3://WORKING!
                $return_query = "SELECT PERMAID, XCOORD, YCOORD from Coordinate where PERMAID in (select PERMAID from Person where USEROAD=1 and USECROSSWALK=1 intersect select PERMAID from Contains where DATE >= " . "'" . $time_start . "'" . " and DATE <= " . "'" . $time_stop . "');";
                break;
            case 4://WORKING!
                $return_query = "SELECT PERMAID, XCOORD, YCOORD from Coordinate where PERMAID in (select PERMAID from Person where USEROAD=1 intersect select PERMAID from Contains where DATE >= " . "'" . $time_start . "'" . " and DATE <= " . "'" . $time_stop . "');";
                break;
        }
    return $return_query;
}
